...
- **字体方案（2026-09 起）**: 日常页面只用 `LXGW WenKai Screen`（霞鹜文楷）一种字体、一个字重，靠字号和颜色分层；金色给日期和数字，玫瑰色给"爱"（心形、今日份爱你）。6000 天纪念页保留原设计用的 Bodoni Moda / Inter / Playfair / Cinzel / Pinyon，只在纪念页打开时加载。
- **玻璃拟态**: `backdrop-filter: blur(20px)` + 半透明白边框，用于所有卡片和面板。
- **动态光效**: 使用 `box-shadow` 和 `mix-blend-mode: overlay` 模拟体积光（Volumetric Lighting）。

### 2026-09-25 首屏改版 + 断行规则（正言）

- **手机不挂 VPN**：熊老婆手机打不开 Google Fonts / jsdelivr / cdnjs，它们在 `<head>` 里会卡住整页。字体全部放进 `fonts/`，Font Awesome 换成内联 SVG 图标，页面零外链。
- **首页**：月洞窗（今天的画隔着窗纸）+ 向日葵星盘（在一起的每一天一颗点，黄金角排列；有画信的日子是贴窗的一圈玫瑰色）。画没上时窗里是一轮满月，文案"今天这封还在路上"。点开时月洞窗放大铺满屏幕再进画廊（0.68 秒）。
- **今日卡横竖同构**：不再按横竖图切两套人格（旧版竖图会变成深蓝金字毛笔版）。画框尺寸由 JS 按画的比例算，卡片高度跟着画走；信笺一栏至少 340px；卡片窄于 720px 时画上信下，竖图限高保证那句话在手机首屏。
- **断行**：所有日期、数字、短语包成 `.ts-u`（nowrap），只在词组之间换行；长句用 `Intl.Segmenter` 按词切，句尾至少 3 个字粘在一起；标题用 `Typeset.fit()` 按栏宽缩字号。验收方法：`?lmdev=1` 下逐封渲染，检查孤字、拆开的单元、溢出（276 封 × 6 种屏宽）。
- **看画页（手机）**：画和信笺连成一组居中，四周是同一幅画的深色晕染；信笺不再绝对定位盖在画上。桌面右栏短句居中、长文从顶部开始滚动（`margin: auto 0` 代替 `justify-content: center`，后者会把长文开头推出滚动区）。
- **踩坑：量宽度要扣掉 transform**。看画页打开有 `scale(0.98)` 动画，`getBoundingClientRect()` 在动画途中量出来偏小，标题字号就定大了（375 宽手机上"街头粉彩+粗粝墙面的风格"超出 2px）。`Typeset.fit()` 现在用 `getBoundingClientRect().width / offsetWidth` 求出缩放比再换算。
- **图片没加载完也要先定版式**：今日卡在画加载前按 4:3 占位、先判定上下 / 左右布局，否则手机慢网时会先按桌面尺寸撑开再跳回来。
